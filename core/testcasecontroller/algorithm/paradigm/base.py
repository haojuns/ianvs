...
class ParadigmBase:
    ...
    def __init__(self, workspace, **kwargs):
        self.modules = kwargs.get("modules")#是一系列module_type对应的module
    ...
    def build_paradigm_job(self, paradigm_type):
        ...
        if paradigm_type == ParadigmType.SINGLE_TASK_LEARNING.value:#如果判断是单任务学习
            return self.module_instances.get(ModuleType.BASEMODEL.value)#那么就返回单任务学习需要的算法模块，但不会指定运行顺序（终身学习：还有任务定义等等模块）
