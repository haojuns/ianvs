...
from datasets import research_campus
...
num_classes = 31 # 这个和train.py的num_classes影响最后的评估程序；new_campuse1中的num_classes影响label识别；模型的num_classes影响模型结构
ignore_label = 255
...
def setup_loaders(args):
    ...
    if 'research_campuse1' in args.dataset:
        dataset = research_campus
        ...
        if args.class_uniform_pct:
            ...
            train_set = dataset.CampusE1Uniform(
                campus_mode, args.weather, args.maxSkip,
                joint_transform_list=train_joint_transform_list,
                transform=train_input_transform,
                target_transform=target_train_transform,
                target_aux_transform=target_aux_train_transform,
            ...
            train_set = dataset.CampusE1(
                ...
                spliting_method=args.spliting_method)

        train_sets.append(train_set)
        # print(len(train_set))
        
        # val_set = dataset.CampusE1('train', 'all', 0,
        #                     transform=val_input_transform,
        #                     target_transform=target_transform,
        #                     cv_split=args.cv,
        #                     image_in=args.image_in,
        ...
        #                     spliting_method=args.spliting_method)
        # val_sets.append(val_set)
        # val_dataset_names.append('Research_CampusE1_Trainingsets_' + args.spliting_method + "_" + args.spliting)
        
        envs = ['E1G', 'E1_2F', 'E1_1F', "all"]
        # envs = ['E1_1F']
        for env in list(envs):
            val_set = dataset.CampusE1('val', env, 0,
                                transform=val_input_transform,
                                target_transform=target_transform,
                                cv_split=args.cv,
                                image_in=args.image_in,
                                split=args.split)
            val_sets.append(val_set)
            val_dataset_names.append('Research_CampusE1_' + env)
    ...
    return train_loaders, val_loaders, train_set, extra_val_loader, covstat_val_loader
