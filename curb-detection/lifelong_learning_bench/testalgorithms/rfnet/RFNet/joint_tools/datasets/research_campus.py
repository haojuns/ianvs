...
import datasets.research_campus_labels as campus_labels
import scipy.misc as m

from config import cfg

trainid_to_name = campus_labels.trainId2name
color_to_trainid = campus_labels.color2trainId
num_classes = 31
ignore_label = 255
# root = cfg.DATASET.campusE1_DIR
root = cfg.DATASET.Research_CampusE1_DIR
...
def add_items(items, aug_items, videos, img_path, mask_path, mask_postfix, mode, maxSkip):
    ...
    for v in videos:
        #v_items = [name.split(img_postfix)[0] for name in
                   #os.listdir(os.path.join(img_path, v.split('/')[0],'folder', v.split('/')[1]))]
        
        #for it in v:
            item = (os.path.join(img_path, v.split('/')[0],'folder',(v.split('/')[1]).split('.')[0]+img_postfix),
                    os.path.join(mask_path, v.split('/')[0],'folder',(v.split('/')[1]).split('.')[0]+mask_postfix))
            ########################################################
...
def make_cv_spliting_splits(img_path, mask_path, spliting, split=None):#for training
    """
    Create splits of train/val data.
    A split is a lists of videos.
    """
    #img_path:'/data/user8302433/fc/ianvs/dataset/campus_split_Style/rgb/split_1'
    #mask_path:'/data/user8302433/fc/ianvs/dataset/campus_split_all/gt/all'
    trn_path = os.path.join(img_path, 'train','folder')
    val_path = os.path.join(mask_path, 'val','folder')
...
def make_cv_splits(img_dir_name, weather, split=None):
    """
    Create splits of train/val data.
    A split is a lists of videos.
    """
    trn_path = os.path.join(root, img_dir_name, weather, 'train', 'folder')
    val_path = os.path.join(root, img_dir_name, weather, 'val', 'folder')
...
def make_dataset(mode, weather, maxSkip=0, fine_coarse_mult=6, cv_split=0, split=None, spliting=None, spliting_method=None):
    """
    ...
    assert weather in ['E1G', 'E1_2F', 'E1_1F', "all"]
    img_dir_name = 'rgb'
    ...
    if mode == 'train':
        spliting_root = root + "_" + spliting_method
        all_root = root +"_all"

        img_path = os.path.join(spliting_root, img_dir_name, spliting)
        mask_path = os.path.join(all_root, 'gt', weather)
        cv_splits = make_cv_spliting_splits(img_path, mask_path, spliting)
        ...
        if weather == "all":
            for weather in ['E1G', 'E1_2F', 'E1_1F']:
    ...
    # logging.info('Research_CampusE1-{}: {} images'.format(mode, len(items)))
    logging.info('Research_CampusE1-{}-{}: {} images'.format(mode, weather, len(items) + len(aug_items)))
...
class CampusE1Uniform(data.Dataset):
    ...
    def __init__(self, mode, weather, maxSkip=0, joint_transform_list=None, sliding_crop=None,
        ...
        self.imgs, self.aug_imgs = make_dataset(mode, self.weather, self.maxSkip, cv_split=self.cv_split, split=self.split)
        assert len(self.imgs), 'Found 0 images, please check the data set'

        # Centroids for fine data
        json_fn = 'Research_CampusE1_new_{}_cv{}_tile{}_{}.json'.format(
...
class CampusE1(data.Dataset):
    ...
    def __init__(self, mode, weather, maxSkip=0, joint_transform=None, sliding_crop=None,
                 ...
                 spliting=None, spliting_method=None):
        self.mode = mode
        self.weather = weather
        self.maxSkip = maxSkip
        ...
        self.imgs, _ = make_dataset(mode, self.weather, self.maxSkip, cv_split=self.cv_split, split=self.split, spliting=self.spliting, spliting_method=spliting_method)
        if len(self.imgs) == 0:
