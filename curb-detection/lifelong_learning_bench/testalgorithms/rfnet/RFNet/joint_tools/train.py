...
from utils.misc import AverageMeter, prep_experiment, evaluate_eval, evaluate_eval_dur_train, fast_hist, save_best_acc
import datasets
import loss
import network
import optimizer
        ...
        while i < max_iter:
            ...
            if epoch % 10 == 9:
                for dataset, val_loader in val_loaders.items():
                    if dataset == "Research_CampusE1_" + args.weather:
...
def train(train_loaders, net, optim, curr_epoch, writer, scheduler, max_iter):
    """
    ...
    for i in range(len(train_loaders[0])):
        ...
        for dataset_index in range(len(train_loader_iters)):
            ...
            domain_label = dataset_index * torch.ones(input.shape[0], dtype=torch.long)
            ...
            if args.use_isw:
                ...
                outputs = net(input, gts=gt, aux_gts=aux_gt, img_gt=img_gt, visualize=args.visualize_feature, dsbn=args.dsbn, mode='train', domain_label=domain_label)
            ...
            if args.local_rank == 0:
                if i % 8 == 7:
                    ...
                    msg = '[epoch {}], [iter {} / {} : {}], [loss {:0.6f}], [for dataset_index: {}], [lr {:0.6f}], [time {:0.4f}]'.format(
...
def validate_during_train(val_loader, dataset, net, criterion, optim, scheduler, curr_epoch, writer, curr_iter, save_pth=True):
    """
...
def validate(val_loader, dataset, net, criterion, optim, scheduler, curr_epoch, writer, curr_iter, save_pth=True):
    ...
    for val_idx, data in enumerate(val_loader): # 一张一张来的
        ...
        with torch.no_grad():
            if args.use_wtloss:
                ...
                output, style_code = net(inputs, dsbn=args.dsbn, mode='val', domain_label=domain_label) # 推理
    ...
    __writer = pd.ExcelWriter('./rfnet_31_Research_CampusE1_' + args.spliting_method + "_" + args.spliting + '.xlsx')
    ...
    # _writer = pd.ExcelWriter('./rfnet_31_Research_CampusE1_' + args.spliting_method + "_" + args.spliting + '_wo_classuniform.xlsx') # 写入Excel文件
