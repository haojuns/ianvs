...
class E1Segmentation(data.Dataset):
    NUM_CLASSES = 24 # 25
    ...
    def __init__(self, args, root=Path.db_root_dir('e1'), data=None, split="train"):
        ...
        self.disparities_base = os.path.join(self.root, self.split, "depth", "e1_real")
    ...
    def __getitem__(self, index):
        ...
        #print(self.transform_tr(sample)['label'].size())
...
if __name__ == '__main__':
    ...
    e1_train = E1Segmentation(args, split='train')

    dataloader = DataLoader(e1_train, batch_size=2, shuffle=True, num_workers=2)
    ...
    for ii, sample in enumerate(dataloader):
        for jj in range(sample["image"].size()[0]):
            ...
            segmap = decode_segmap(tmp, dataset='e1')
