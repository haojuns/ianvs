...
def make_data_loader(args, train_data=None, valid_data=None, test_data=None, **kwargs):
    ...
    if args.dataset == 'e1':
        if train_data is not None:
            train_set = e1.E1Segmentation(args, data=train_data, split='train')
            num_class = train_set.NUM_CLASSES
            train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, **kwargs)
        else:
            train_loader, num_class = None, e1.E1Segmentation.NUM_CLASSES

        if valid_data is not None:
            val_set = e1.E1Segmentation(args, data=valid_data, split='val')
            num_class = val_set.NUM_CLASSES
            val_loader = DataLoader(val_set, batch_size=args.val_batch_size, shuffle=False, **kwargs)
        else:
            val_loader, num_class = None, e1.E1Segmentation.NUM_CLASSES

        if test_data is not None:
            test_set = e1.E1Segmentation(args, data=test_data, split='test')
            num_class = test_set.NUM_CLASSES
            test_loader = DataLoader(test_set, batch_size=args.test_batch_size, shuffle=False, **kwargs)
        else:
            test_loader, num_class = None, e1.E1Segmentation.NUM_CLASSES
