...
import sys
sys.path.append('examples/curb-detection/lifelong_learning_bench_ours/testalgorithms/rfnet/RFNet')
...
class Trainer(object):
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            import pdb 
            ##pdb.set_trace()
            loss = self.criterion(output, target)#正常的情况:output.shape,target.shape:  (torch.Size([8, 24, 768, 768]), torch.Size([8, 768, 768]))                   #错误的情况： (torch.Size([8, 24, 768, 768]), torch.Size([8, 768, 768, 3]))
            loss.backward()
            self.optimizer.step()
            #print(self.optimizer.state_dict()['param_groups'][0]['lr'])
